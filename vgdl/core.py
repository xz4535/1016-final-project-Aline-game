...
class VGDLSprite(object):
    ...
    def _updatePos(self, orientation, speed=None):
      ...
      # Check if orientation is already in the correct format (tuple) and bypass the mapping
      if isinstance(orientation, tuple) and len(orientation) == 2:
          orientation_x, orientation_y = orientation
      else:
          # Convert string orientation to tuple using a direction map
          direction_map = {
              'UP': (0, -1),
              'DOWN': (0, 1),
              'LEFT': (-1, 0),
          ...
          # Ensure orientation is a string and map it
          try:
              if isinstance(orientation, str):
                  orientation = orientation.upper()
                  orientation_x, orientation_y = direction_map[orientation]
              else:
                  raise ValueError("Orientation must be a string or a tuple of two integers.")
          except KeyError:
              print(f"Invalid orientation: {orientation}")
              return  # Exit if the orientation is not valid

      # Perform the move if cooldown is over
      if (self.lastmove+1) % self.cooldown == 0 and (abs(orientation_x) + abs(orientation_y)) != 0:
          self.rect = self.rect.move((orientation_x * speed, orientation_y * speed))
